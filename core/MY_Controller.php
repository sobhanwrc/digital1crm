...
//                exit;
            ...
            $select = "category_type,firm_customization";
            $cond = "group by category_type";
            $all_category = $this->codes_model->fetch($cond, $select);
//t($all_category);
            $category_code_combination = array();

            if ($role_code == 'SITEADM') { // site admin
                foreach ($all_category as $key => $value) {
                    $category_type = $value['category_type'];
                    $cond = "AND category_type='$category_type'";
                    $select = "code,short_description";
                    $all_codes = $this->codes_model->fetch($cond, $select);
                    $category_code_combination[$category_type] = $all_codes;
                }
            } elseif ($role_code == 'FIRMADM') { // firm admin
                foreach ($all_category as $key => $value) {

                    $category_type = $value['category_type'];
                    $firm_customization = $value['firm_customization'];
                    if ($firm_customization) {
                        $sql = "SELECT 
                plma_codes.category_type,
                plma_codes.code,
                plma_firm_codes.short_description
           FROM plma_codes, plma_firm_codes
           WHERE plma_codes.code_seq_no = plma_firm_codes.code_seq_no
           AND plma_firm_codes.firm_seq_no = $firm_seq_no AND plma_codes.category_type='$category_type'";
                        $query = $this->db->query($sql);
                        $all_codes = $query->result_array();
                        //                        echo $this->db->last_query();
                        //                        exit;
                    } else {
                        $cond = "AND category_type='$category_type'";
            ...
            } elseif ($role_code == 'ATTRMGR' || $role_code == 'ATTR') {
                foreach ($all_category as $key => $value) {
...
//                }
            ...
            $this->data['codes'] = $category_code_combination;
            //            echo $this->db->last_query();
//                        t($category_code_combination);
//                        exit;
        }
...
left join `plma_codes` `pcodes` on `pcodes`.`code` = `pfirm`.firm_jurisdiction ";
        ...
        $query = $this->db->query($sql);
        $this->data['all_firms'] = $query->result();

        //t($query->result(), 1);
    }

//    function getShortDescByCode($code='',$firm_seq_no=''){
//        
...
//        $admin_all_session = $this->session->userdata('admin_session_data');
    ...
    public function email_config($param = array()) {
        ...
        $config['smtp_user'] = "jayanta.wrc@gmail.com";
        $config['smtp_pass'] = "wrc@2016!";
        $config['charset'] = "utf-8";
        $config['mailtype'] = "html";
        $config['newline'] = "\r\n";
        $config['validation'] = TRUE;

        echo $this->email->initialize($config);
