...
//       //################# Activity Goals Fetch #######################//
        ...
        $sql11 = "SELECT DISTINCT pact.activity_goal "
                . "FROM plma_activity as pact "
//                . "LEFT JOIN plma_activity_dtl pad ON pact.activity_seq_no = pad.activity_seq_no "
                . "WHERE pact.firm_seq_no = '" . $firm_seq_no . "'";  
...
//            echo 123; exit;
        ...
        foreach ($res11 as $key => $value) {
           ...
           $activity_goal = $value['activity_goal'];
            ...
            $sql_1 = "SELECT pabd.budget_code, pa.activity_name, pa.origin_attorney_seq_no, pa.activity_goal, pad.activity_seq_no, pabd.budget_code_type, pabd.activity_dtl_seq_no, "
...
//            t($bud_details_type); 
//            exit; 
//            $bud_details_type['origin_attorney_seq_no'] = $value['origin_attorney_seq_no'];
...
//        exit;
        ...
        $sql_2 = "SELECT pabd.budget_code, pa.activity_name, pa.origin_attorney_seq_no,
        ...
        $row_sum = $qry_2->result_array();
        $bud_details_sum_final = array();
        foreach ($res1 as $key2 => $value2) {
            $bud_details_sum = array();
            foreach ($row_sum as $key1 => $value1) {
                if ($value2['code'] == $value1['budget_code_type']) {
...
//        exit;
        ...
        /******************************* WHEN ATTORNEY LOGS IN ****************************************/ 
        
        else if($role_code == 'ATTR'){

        //######################## Activity Goal-wise budget hour/cost fetch and sum - Begin #####################// 

        $bud_details_final_attr = array();
//        t($res11); exit;
        foreach ($res11 as $key => $value) {
            
            $activity_goal = $value['activity_goal'];
            
            $sql_activity_fetch = "SELECT pact.activity_seq_no, pact.origin_attorney_seq_no, pact.activity_name, pact.activity_goal, pact.activity_type "
                    . "FROM plma_activity pact "
//                    . "LEFT JOIN plma_act_attorney pat ON pat.activity_seq_no = pact.activity_seq_no "
                    . "WHERE pact.activity_goal = '" . $activity_goal . "' ";
            $query_activity_fetch = $this->db->query($sql_activity_fetch);
            $row_activity_fetch = $query_activity_fetch->result_array();
//           echo $this->db->last_query();
//            t($row_activity_fetch);
            $bud_details_final = array();
            foreach($row_activity_fetch as $key_activity => $value_activity){
              $activity_seq_no = $value_activity['activity_seq_no'];
//              $activity_name = $value_activity['activity_name'];
  
              $sql_attorney_fetch = "SELECT DISTINCT pad.attorney_seq_no "
                      . "FROM plma_activity_dtl as pad "
                      . "LEFT JOIN plma_activity pact ON pad.activity_seq_no = pact.activity_seq_no "
                      . "WHERE pad.activity_seq_no = '" . $activity_seq_no . "' "; 
             $query_attorney_fetch = $this->db->query($sql_attorney_fetch);
            $row_attorney_fetch = $query_attorney_fetch->result_array();
//            echo $this->db->last_query();
//            t($row_attorney_fetch);
            foreach ($row_attorney_fetch as $key_attorney => $value_attorney){
                
               $attorney_seq_no = $value_attorney['attorney_seq_no'];

                $sql_1 = "SELECT pabd.budget_code, SUM(pabd.budget_code_hours) as total_hour, SUM(pabd.budget_code_cost) as total_cost, pabd.budget_code_type, pad.attorney_seq_no, pact.activity_seq_no, pact.activity_name, pact.activity_goal "
                        ...
                        . "WHERE pad.attorney_seq_no = '" . $attorney_seq_no . "' AND pact.activity_goal = '" . $activity_goal . "' "
                        . "GROUP BY pabd.budget_code, pact.activity_goal";
...
//                $bud_details_type['activity_goal'] = $value['activity_goal'];
                ...
                $bud_details_final[$value_activity['activity_name']] = $bud_details_type; 
//                $bud_details_final['activity_goal'] = $value['activity_goal'];
            }
            
            $bud_details_final_attr[$value['activity_goal']] = $bud_details_final;                                                                                                                                          /****** activity goal on key (from activity table data fetch) ******/
        }
//        t($bud_details_final_attr);
//        exit;
        //######################## Activity Goal-wise budget hour/cost fetch and sum - End #########################//
        //################# Sum of Total Hours/Total cost of all the activities under a firm - BEGIN  ##################//    
        ...
        $sql_2 = "SELECT pabd.budget_code, pa.activity_name, pa.origin_attorney_seq_no, "
            ...
            ."LEFT JOIN plma_activity_dtl pad ON  pabd.activity_dtl_seq_no = pad.activity_dtl_seq_no "
            ."LEFT JOIN plma_activity pa ON pad.activity_seq_no = pa.activity_seq_no "
            ."WHERE pa.firm_seq_no = '" . $firm_seq_no . "'"
            ."GROUP BY pabd.budget_code, pa.activity_seq_no ";
        $qry_2 = $this->db->query($sql_2);
        $row_sum = $qry_2->result_array();
//         t($row_sum);
//        exit;      
...
//            $activity_goal = $value['activity_goal'];
            ...
            $sql_1 = "SELECT pabd.budget_code, pa.activity_name, pa.activity_goal, pad.activity_seq_no, pabd.budget_code_type, pabd.activity_dtl_seq_no, "
                    . "SUM(pabd.budget_code_hours) as total_hour, SUM(pabd.budget_code_cost) as total_cost, pad.attorney_seq_no "
                    . "FROM plma_activity_dtl pad "
                    . "LEFT JOIN plma_activity pa ON pad.activity_seq_no = pa.activity_seq_no "
                    . "LEFT JOIN plma_act_bud_dtl pabd ON pabd.activity_dtl_seq_no = pad.activity_dtl_seq_no "
                    . "WHERE pa.firm_seq_no = '" . $firm_seq_no . "' AND pad.activity_seq_no = '" . $activity_seq_no . "' "
//                    . "AND pad.attorney_seq_no = '" .$attorney_seq_no. "' "
                    . "GROUP BY pabd.budget_code, pad.activity_seq_no";
            $qry_1 = $this->db->query($sql_1);
            $row = $qry_1->result_array();
...
//            exit;
        ...
        $sql_2 = "SELECT pabd.budget_code, pa.activity_name, "
                ...
                . "LEFT JOIN plma_activity pa ON pad.activity_seq_no = pa.activity_seq_no "
                . "WHERE pa.firm_seq_no = '" . $firm_seq_no . "' AND pa.activity_goal = '" . $activity_goal . "'"
                . "GROUP BY pabd.budget_code, pa.activity_goal, pad.activity_seq_no";
        $qry_2 = $this->db->query($sql_2);
        $row_sum = $qry_2->result_array();
//        t($row_sum);
//        exit;
...
//              exit;
                ...
                $activity_details_by_activity['activity_name'] = $activity_name;
